...
export const FAQ_ITEMS = [
  {
    q: "What is TokenAnalytics?",
    a: "TokenAnalytics (formerly TokenTelemetry, sometimes misspelled as 'token analitics' or 'tokenanalitics') is a free, open-source, 100% local observability dashboard for AI coding agents like Claude Code, Codex, Gemini CLI, Cursor, and GitHub Copilot. It tracks tokens, cost, tool calls, and reasoning by reading the log files those agents already write — no SDK, no signup, no cloud.",
  },
  {
    q: "How do I track Claude Code token usage?",
    a: "Install TokenAnalytics, run Claude Code normally, and open http://localhost:3000. TokenAnalytics auto-detects Claude Code sessions from ~/.claude/ logs — no instrumentation, no SDK, no config.",
  },
  {
    q: "How do I monitor Google Antigravity, Codex, and Gemini CLI costs?",
    a: "TokenAnalytics auto-reads logs from Google Antigravity (Google's agentic coding CLI), OpenAI Codex CLI, Gemini CLI, Cursor, GitHub Copilot, Qwen CLI, OpenCode, Vibe, and Grok Build (xAI). Token counts and dollar costs appear in the local dashboard automatically.",
  },
  {
    q: "Is there a free tool to monitor AI coding agent token usage?",
    a: "Yes — TokenAnalytics is free, open-source (MIT), and runs 100% locally. No account, no signup, no cloud.",
  },
  {
    q: "Does TokenAnalytics send my data to the cloud?",
  ...
  {
    q: "How does TokenAnalytics compare to Langfuse or Helicone?",
    a: "TokenAnalytics is purpose-built for AI coding agents and is zero-config — no SDK instrumentation. Langfuse and Helicone are general LLM-app observability platforms that require code changes and (typically) a cloud account.",
  },
  {
  ...
  {
    q: "Can I use TokenAnalytics from inside Hermes Dashboard?",
    a: "Yes — there's a Hermes Dashboard plugin that registers a 'TokenAnalytics' tab inside Hermes's web UI at port 9119. It's a thin launcher: deep-link cards open the relevant TokenAnalytics page (Hermes Overview, Skills, Memory, Analytics, Projects) in a new browser tab, so you don't have to remember a second port. Install with `./scripts/install-hermes-plugin.sh` from the TokenAnalytics repo, then run `hermes dashboard`.",
  },
];
