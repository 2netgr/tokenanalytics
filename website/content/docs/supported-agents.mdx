...
description: The 10 coding agents and Hermes autonomous agent that TokenAnalytics tracks, how each is detected, and what data each captures.
---
...
Antigravity is Google's browser automation agent. TokenAnalytics reads its sessions from `~/.gemini/antigravity/` and additionally captures browser screenshots and Antigravity recording strips (thumbnail sequences from `browser_recordings`). See [Artifacts](/docs/features/artifacts).

### Cursor

Cursor stores its chat sessions in `~/.cursor/` and VS Code's `workspaceStorage/`. TokenAnalytics merges sessions from both locations. Plans extracted from Cursor's `/plan` output are available in the Projects → Plans tab.
...
Don't see your agent? [Open a GitHub issue](https://github.com/2netgr/tokenanalytics/issues) with the agent name and its log file format and we'll add support.
