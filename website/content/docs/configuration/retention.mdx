...
description: Configure how long TokenAnalytics keeps session data, enable durable history, and manage transcript archival.
---

import { Callout } from 'fumadocs-ui/components/callout';

By default, TokenAnalytics reads agent session files directly from their log directories on every request. Durable history is an optional mode that caches processed session data in a local SQLite database, enabling longer date ranges in Analytics and the Projects heatmap.

Open **Settings → History & Retention** to configure these options.

## Durable history

When **Durable history** is enabled, TokenAnalytics writes parsed session data to `~/.tokentelemetry/history.db` as sessions are processed. This enables:
...
By default, TokenAnalytics reads session transcripts live from the agent's log directory. If you enable **Transcript archival**, TokenAnalytics copies the full transcript text into `history.db` when a session is first processed.
...
Both are SQLite databases. You can inspect them with `sqlite3` or any SQLite viewer. The original agent log files (`~/.claude/`, `~/.codex/`, etc.) are never modified by TokenAnalytics.
