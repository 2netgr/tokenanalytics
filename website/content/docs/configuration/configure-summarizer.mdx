...
Open Settings → Summarizer in the TokenAnalytics UI to configure your backend.
...
All backends are invoked locally. For CLI-based backends (Claude, Codex, Gemini, Qwen, Antigravity), TokenAnalytics shells out to the CLI binary you already have installed. The OpenAI-compatible backend uses stdlib `urllib` — no new Python dependencies.
...
<Callout type="info">
Non-OpenAI parameters (top_k, min_p, repetition_penalty, enable_thinking) are sent first. If the server rejects them with HTTP 400, TokenAnalytics automatically retries with a clean OpenAI-only payload so the summary still goes through. You don't need to toggle this manually.
</Callout>
...
Set them before launching TokenAnalytics:
