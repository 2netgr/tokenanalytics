---
title: Remote Access
description: Expose the TokenAnalytics dashboard to other devices — tailnet, LAN, or SSH tunnel — with token-based authentication.
---

import { Callout } from 'fumadocs-ui/components/callout';

TokenAnalytics is **local-first** and binds to `127.0.0.1` by default. Remote access is opt-in and token-gated. Loopback requests (your browser on the same machine) are always exempt from the token requirement — the default local experience is unchanged.
