...
description: How TokenAnalytics classifies sessions as subscription, API, local, or unknown — and how to override the detection per agent.
---

TokenAnalytics attaches a **billing mode** to every session to help you understand the nature of the cost. The mode is a label only — it never changes the token math or the API-equivalent cost calculation.
...
TokenAnalytics detects the billing mode for each agent automatically:
...
If you're a Claude Pro subscriber, your actual bill doesn't change based on token usage (within plan limits), but TokenAnalytics still shows you the API-equivalent cost as a reference — it's a useful signal for understanding how much value you're getting from your subscription vs what you'd pay on the API.
