...
Summarization lets you generate a human-readable summary of any session trace without reading through every event manually. You pick a backend (Claude, Codex, Gemini, Ollama, etc.), click Generate, and TokenAnalytics sends the session text to the model and displays the result.
...
If a summary fails, TokenAnalytics shows a structured error card — never a raw stack trace or provider JSON blob. Each card contains:
