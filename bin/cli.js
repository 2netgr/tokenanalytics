#!/usr/bin/env node
/**
 * TokenAnalytics — single cross-platform entry point.
 *
...
// --- collector subcommand parsing ----------------------------------------
// `node bin/cli.js collector --hub-url <URL> --auth-token <T> [--interval <N>]`
// scans this machine's local logs and syncs summary ROLLUPS ONLY to a hub Mac.
// Mirrors parseArgs' `--flag value` / `--flag=value` style.
function parseCollectorArgs(argv) {
  const out = { hubUrl: '', authToken: '', interval: 60 };
  const take = (i) => {
    if (i + 1 >= argv.length) die(`expected a value after ${argv[i]}`);
    return argv[i + 1];
  };
  const setInterval = (raw) => {
    const n = parseInt(raw, 10);
    if (!Number.isFinite(n) || n < 1) die(`invalid interval: ${raw}`);
    out.interval = n;
  };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '-h' || a === '--help') { printHelp(); process.exit(0); }
    else if (a === '--hub-url')              { out.hubUrl = take(i); i++; }
    else if (a.startsWith('--hub-url='))     { out.hubUrl = a.slice('--hub-url='.length); }
    else if (a === '--auth-token')           { out.authToken = take(i); i++; }
    else if (a.startsWith('--auth-token='))  { out.authToken = a.slice('--auth-token='.length); }
    else if (a === '--interval')             { setInterval(take(i)); i++; }
    else if (a.startsWith('--interval='))    { setInterval(a.slice('--interval='.length)); }
    else die(`unknown argument: ${a}\nRun with --help for usage.`);
  }
  if (!out.hubUrl.trim()) die('collector requires --hub-url <URL> (the hub Mac\'s base URL).');
  return out;
}
...
function printHelp() {
  console.log([
    'Usage: tokenanalytics [options]',
    '',
    'Options:',
    '  -p, --port <N>            Frontend (Next.js) port. Default 3000.',
    '  -a, --api-port <N>        Backend (FastAPI) port. Default 8000.',
    '  -d, --data-dir <P>        Where TokenAnalytics stores its config + state.',
    '                            Default ~/.tokenanalytics (sets TOKENANALYTICS_DATA_DIR).',
    ...
    '      --auth-token <T>      Bearer token the hub requires for remote callers.',
...
function startCollector(argv) {
  const { hubUrl, authToken, interval } = parseCollectorArgs(argv);

  console.log('\nTokenAnalytics — collector');
  console.log('--------------------------');
  checkNode();
  // Reuse the backend venv bootstrap (creates venv + installs deps if needed).
  // The collector never needs the frontend, so ensureFrontend() is skipped.
  ensureBackend();

  console.log('\n→ syncing summary rollups only — no transcripts or prompt text');
  console.log(`Hub:        ${hubUrl}`);
  console.log(`Interval:   ${interval}s`);
  console.log(`Auth:       ${authToken ? 'bearer token set' : 'none (loopback/insecure hub only)'}`);
  console.log('Press Ctrl+C to stop.\n');

  // Hand config to collector.py via env (TA_* mirrors device_identity's vars).
  // TA_DEVICE_ROLE=collector marks this process; collector.py also persists it.
  const collector = spawn(venvPython, ['collector.py'], {
    cwd: backendDir,
    stdio: 'inherit',
    detached: !isWindows,
    env: {
      ...process.env,
      TA_HUB_URL: hubUrl,
      TA_AUTH_TOKEN: authToken,
      TA_INTERVAL: String(interval),
      TA_DEVICE_ROLE: 'collector',
    },
  });
  ...
  const shutdown = (code = 0) => {
    ...
    if (collector && !collector.killed) {
      try {
        if (isWindows) {
          spawnSync('taskkill', ['/pid', String(collector.pid), '/T', '/F']);
        } else {
  ...
  collector.on('exit', (code) => shutdown(code || 0));
}

// One-time data-dir migration (TokenTelemetry → TokenAnalytics): if a legacy
// ~/.tokentelemetry exists but ~/.tokenanalytics doesn't (and no path is pinned),
// copy the data into the new home before the backend touches it. Best-effort and
// silent unless it actually migrates; the legacy dir is left intact as a backup.
function maybeMigrateDataDir(env) {
  try {
    const r = spawnSync(
      venvPython,
      ['-c', 'import tt_paths,sys; sys.stdout.write("1" if tt_paths.migrate_legacy_data_dir() else "")'],
      { cwd: backendDir, encoding: 'utf8', env: env || process.env },
    );
    if (r && r.stdout && r.stdout.includes('1')) {
      console.log('→ migrated existing data from ~/.tokentelemetry to ~/.tokenanalytics');
    }
  } catch (_) { /* non-fatal — resolver still adopts the legacy dir */ }
}
...
async function start() {
  ...
  const backendEnv = dataDir
    ? { ...process.env, TOKENANALYTICS_DATA_DIR: dataDir }
    : process.env;
  ...
  const backend = spawn(venvPython, ['main.py', '--port', String(apiPort), '--host', host], {
    ...
    detached: !isWindows,
    // backendEnv carries TOKENANALYTICS_DATA_DIR when --data-dir is set.
...
// Dispatch on the first positional. `collector` runs collector mode; any other
// invocation (including no args) falls through to the unchanged default start().
const argv = process.argv.slice(2);
if (argv[0] === 'collector') {
  startCollector(argv.slice(1));
} else {
  start();
}
