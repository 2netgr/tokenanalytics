"""Product telemetry — REMOVED / hard-disabled in this build (nothing is sent).
...
# domain is attached, switch this to "https://telemetry.tokenanalytics.app/e".
# --------------------------------------------------------------------------
...
def enabled() -> bool:
    """Telemetry was REMOVED from this build (privacy — nothing leaves the machine).
    Hard-disabled: always False, so every ``emit()`` is a guaranteed no-op regardless
    of preference, env, or CI. ``env_forced_off`` / ``_is_ci`` are kept only for the
    Settings panel and the redaction tests; they no longer gate anything."""
    return False
...
def _post(payload: Dict[str, Any]) -> None:
    ...
    """
    return  # Telemetry removed — never transmit. Unreachable behind enabled()==False; a hard stop.
    try:
...
def preview() -> Dict[str, Any]:
    ...
    return {
        "enabled": False,   # telemetry removed in this build — nothing is collected or sent
