...
# Cursor agent-transcripts carry NO usage/token fields, but assistant/user lines
# do carry message.content text. We estimate tokens from text length (~4 chars
# per token) so cursor sessions aren't silently reported at 0 tokens / $0.00.
def _cursor_text_len(content) -> int:
    """Sum the character length of all text in a cursor ``message.content``.

    ``content`` may be a plain string or a list of ``{type:"text", text:"..."}``
    items (other item types are ignored)."""
...
# Cursor reports its own Composer model under several aliases that don't resolve
# in the pricing table; fold the fast/auto variants onto the canonical
# "composer-2.5" id. Claude-backed models are left untouched so they hit their
# existing Anthropic pricing entries.
...
def _normalize_cursor_model(model: Optional[str]) -> str:
    """Resolve a cursor ``message.model`` to a priceable model id.

    Defaults to ``composer-2.5`` when no model was seen on the transcript."""
    if not model:
        return "composer-2.5"
    return _CURSOR_MODEL_ALIASES.get(model.strip().lower(), model)
...
def _scan_sessions_sync():
                ...
                for line in f:
                    try:
                        ...
                        if sid not in claude_sessions:
                            # No transcript file on disk → do NOT fabricate a phantom
                            # session. history.jsonl lists every session id ever seen,
                            # but most of those transcripts have been pruned by Claude
                            # (cleanupPeriodDays). Their durable record lives in the
                            # history store (history.db, ADR-0002), captured while the
                            # file existed; the live scan only reports what is still on
                            # disk. Creating zero-token stubs here inflated the session
                            # count by tens of thousands and (worse) pushed real files
                            # out of the parse window.
                            continue
                        # Overlay metadata onto the real, file-backed session.
    ...
    # Parse tokens for EVERY file-backed transcript. A prior `[:100]` recency
    # slice here capped token parsing to 100 sessions; combined with the fileless
    # history.jsonl stubs (which rank by their own timestamps), real transcripts
    # were pushed past rank 100 and never read — recording multi-hundred-million-
    # token sessions as 0. The per-file cost is bounded by the transcripts on disk.
    if claude_sessions:
    ...
    if codex_index.exists() or (CODEX_DIR / "sessions").is_dir():
        ...
        # Parse every file-backed session (same fix as the Claude path: a `[:100]`
        # cap here would drop real rollouts' tokens once a user exceeds 100 sessions).
                    ...
                    if trans_dir.is_dir():
                        ...
                        if cf.exists():
                            try:
                                ...
                                plans = []
                                model = None
                                # Cursor transcripts carry no usage fields; tally
                                # user/assistant text length so we can estimate.
                                user_chars = 0
                                assistant_chars = 0
                                with open(cf, "r", encoding="utf-8", errors="replace") as f:
                                    for line in f:
                                        ...
                                        msg = data.get("message", {}) if isinstance(data.get("message"), dict) else {}
                                        # Last non-empty model wins (the active model
                                        # may switch mid-session; falls back later).
                                        if msg.get("model"): model = msg.get("model")
                                        if data.get("role") == "user":
                                            c = msg.get("content", [])
                                            user_chars += _cursor_text_len(c)
                                            if not first_msg:
                                        ...
                                        if data.get("role") == "assistant":
                                            assistant_chars += _cursor_text_len(msg.get("content"))
                                            usage = msg.get("usage", {}) if isinstance(msg.get("usage"), dict) else {}
                                            cr = usage.get("cache_read_input_tokens", 0) or 0
                                            cc = usage.get("cache_creation_input_tokens", 0) or 0
                                            cc_1h = (usage.get("cache_creation", {}) or {}).get("ephemeral_1h_input_tokens", 0) or 0
                                ...
                                # Cursor records no usage, so the parse loop above
                                # leaves tokens at 0. In that (normal) case estimate
                                # from text length (~4 chars/token) and mark it so the
                                # UI can flag the numbers as approximate.
                                if not (tokens["input"] or tokens["output"] or tokens["cached"]):
                                    model = _normalize_cursor_model(model)
                                    tokens["input"] = (user_chars + 3) // 4
                                    tokens["output"] = (assistant_chars + 3) // 4
                                    tokens["cached"] = 0
...
@app.get("/sessions")
async def get_sessions(fresh: bool = False, device: str = "local"):
    """Return the session list. Pass ?fresh=1 to force a re-scan.

    ``device`` scopes the result across synced devices (ADR-0004): ``local``
    (default) is the live local scan, byte-for-byte as before; ``all`` merges
    live-local with every synced device's stored rollups (live wins for local);
    a concrete device_id returns just that device's stored rollups."""
    live = await get_sessions_cached(fresh=fresh)
    if not device or device == "local":
        return live
    import history_store, device_identity
    local_id = device_identity.device_id()
    if device == local_id:
        return live
    if device == "all":
        merged: Dict[tuple, Dict[str, Any]] = {
            (s.get("device_id"), s["agent"], s["id"]): s for s in history_store.query()
        }
        for s in live:
            merged[(local_id, s.get("agent"), s.get("id"))] = s  # live wins for local
        return list(merged.values())
    return history_store.query(device_ids=[device])


# ── multi-device sync (ADR-0004) ─────────────────────────────────────────────

class DeviceRegisterIn(BaseModel):
    device_id: str
    device_name: Optional[str] = None
    device_role: Optional[str] = "collector"


class SessionRollupIn(BaseModel):
    """A single session rollup as it crosses the network: raw token/cost facts
    only. ``extra='forbid'`` rejects any smuggled transcript/prompt/output field,
    enforcing the summary-only sync policy at the schema boundary."""
...
class SyncSessionsIn(BaseModel):
    device_id: str
    device_name: Optional[str] = None
    device_role: Optional[str] = "collector"
    sessions: List[SessionRollupIn] = []


@app.get("/devices")
async def list_devices_endpoint():
    """Known devices + this machine's identity (ADR-0004). Reads are gated for
    remote callers by the bearer-token middleware; loopback is exempt."""
    import history_store, device_identity
    local = device_identity.local_device()
    # Ensure the local device always appears in the registry, even with 0 sessions.
    history_store.register_device(
        local["device_id"], local["device_name"], local["device_role"],
        source_origin="local_scan", last_seen_at=local["last_seen_at"],
    )
    return {"local": local, "devices": history_store.list_devices()}


@app.post("/devices/register")
async def register_device_endpoint(body: DeviceRegisterIn):
    """Register/refresh a collector device. Remote callers are gated by the
    bearer-token middleware; loopback (the hub itself) is exempt."""
    from fastapi import HTTPException
    if not (body.device_id or "").strip():
        raise HTTPException(status_code=400, detail="device_id is required")
    import history_store
    history_store.register_device(
        body.device_id.strip(), body.device_name, body.device_role,
...
async def sync_sessions_endpoint(body: SyncSessionsIn):
    """Authenticated collector endpoint: accept session *rollups only* and store
    them under the remote device_id with ``source_origin='remote_sync'``.

    Auth is enforced upstream by the bearer-token middleware — an unauthenticated
    *remote* request never reaches here. Transcript/prompt smuggling is rejected
    by ``SessionRollupIn``'s ``extra='forbid'`` (defense in depth)."""
    from fastapi import HTTPException
    if not (body.device_id or "").strip():
        raise HTTPException(status_code=400, detail="device_id is required")
    import history_store
    rows: List[Dict[str, Any]] = [s.model_dump() for s in body.sessions]
    now = datetime.now(timezone.utc).isoformat()
    did = body.device_id.strip()
    history_store.register_device(
        did, body.device_name, body.device_role,
        source_origin="remote_sync", last_seen_at=now,
    )
    stored = history_store.upsert_sessions(
        rows, device_id=did, source_origin="remote_sync",
        device_name=body.device_name, device_role=body.device_role,
    )
...
async def sync_pull_endpoint(
    ...
    """Pull feed for bidirectional sync (ADR-0004): every stored rollup whose
    ``last_seen_at`` is newer than ``since`` (or all of them when omitted),
    excluding ``exclude_device``'s own rows, capped at ``limit`` and ordered by
    the write watermark ascending. Returns the rows plus ``server_time`` (UTC ISO
    now) so the caller can advance its watermark monotonically.

    Auth is enforced upstream by the bearer-token middleware — an unauthenticated
    remote request never reaches here; loopback is exempt."""
    import history_store
    rows = history_store.query_since(since, exclude_device, limit)
    return {"sessions": rows, "server_time": datetime.now(timezone.utc).isoformat()}
...
async def get_sync_config():
    """Current sync settings WITHOUT the raw token — a credential is never echoed
    back over the network. Reports only ``has_token`` so the UI can show whether a
    token is set."""
    import sync_config
...
async def post_sync_config(body: SyncConfigIn):
    """Merge + persist sync settings, then restart the worker so the change takes
    effect immediately. A blank/omitted ``auth_token`` keeps the stored one (so the
    UI can update the hub URL without re-entering the token). Returns {ok: true}."""
    import sync_config, sync_worker
    cfg = sync_config.load()
    if body.hub_url is not None:
        cfg["hub_url"] = body.hub_url.strip()
    # An empty/omitted token preserves the existing one; a non-blank value replaces it.
    if body.auth_token:
...
async def post_sync_now():
    """Run one PUSH+PULL cycle immediately (the UI "Sync now" button) and return
    the resulting status."""
    import sync_worker
    return sync_worker.trigger_once()


@app.get("/sync/status")
async def get_sync_status():
    """Current sync status (counts, timestamps, last error). The auth token is
    never included."""
    import sync_worker
...
async def get_projects(include_hidden: bool = False):
    ...
    for s in sessions:
        ...
        if proj not in projects:
            ...
            projects[proj] = {"name": proj_name, "path": proj, "session_count": 0, "agents": set(), "mcp_tools": set(), "subagent_count": 0, "plan_count": 0, "tokens": {"input": 0, "output": 0, "cached": 0, "total": 0, "cost": 0.0}, "plans": [], "last_activity": None}
        projects[proj]["session_count"] += 1; projects[proj]["agents"].add(s["agent"])
        ts = s.get("timestamp")
        if ts:
            ts_str = str(ts)
            if projects[proj]["last_activity"] is None or ts_str > projects[proj]["last_activity"]:
                projects[proj]["last_activity"] = ts_str
...
async def apply_update(request: Request):
    """One-click self-update: pull the latest code from the git remote and restart,
    so the user doesn't run `git pull` by hand. LOOPBACK-ONLY — only the operator's
    own dashboard on this machine may trigger it (it runs git and restarts the
    process). Auto-restart relies on the app being supervised (the LaunchAgent's
    KeepAlive): the backend exits after pulling and is relaunched fresh with the new
    code + any reinstalled deps."""
    from fastapi import HTTPException
    import subprocess as _sp, threading as _th, os as _os
    client = request.client.host if request.client else None
    if not _is_loopback(client):
        raise HTTPException(status_code=403, detail="Updates can only be applied from this device.")
    before = _local_commit()
    try:
        r = _sp.run(["git", "pull", "--ff-only"], cwd=str(_REPO_ROOT),
                    capture_output=True, text=True, timeout=120)
    except Exception as e:  # noqa: BLE001
        raise HTTPException(status_code=500, detail=f"git pull failed: {e}")
    if r.returncode != 0:
        raise HTTPException(status_code=500, detail=(r.stderr or r.stdout or "git pull failed").strip()[:400])
    after = _local_commit()
    updated = bool(after and after != before)
    if updated:
        # Exiting the backend makes bin/cli.js shut down; the LaunchAgent's
        # KeepAlive relaunches it. Delay briefly so this response is delivered first.
        _th.Timer(1.5, lambda: _os._exit(0)).start()
    return {"ok": True, "updated": updated, "from": before, "to": after,
            "detail": (r.stdout or "").strip()[:400], "restarting": updated}
...
async def _telemetry_startup():
    ...
    try:
        ...
        pass
    # Kick off automatic bidirectional sync if this device is configured for it
    # (ADR-0004). Best-effort: an unconfigured device just stays idle, and any
...
def _resolve_device_filter(device: Optional[str]):
    """Map the UI ``device`` selector to a stored-query allow-list + whether the
    local live scan should be merged in (ADR-0004). ``local`` (default) → this
    machine only; ``all`` → every device; a concrete ``device_id`` → just that
    one. Returns ``(device_ids_or_None, include_local_live)``."""
    local_id = _local_device_id()
    if not device or device == "local":
        return [local_id], True
    if device == "all":
        return None, True
    return [device], (device == local_id)
...
async def get_analytics(
    ...
    device: str = Query("local"),
    include_delegated: bool = Query(False),
):
    import history_store
    ...
    # Multi-device (ADR-0004): scope to the selected device. Default 'local'
    # reproduces the pre-multi-device dashboard exactly. The merge key includes
    # device_id so two Macs sharing an (agent, id) never collapse into one row.
    device_ids, include_local_live = _resolve_device_filter(device)
    local_id = _local_device_id()
    stored = history_store.query(from_b, to_b, agents, models, projects, device_ids=device_ids)
    merged: Dict[tuple, Dict[str, Any]] = {(s.get("device_id"), s["agent"], s["id"]): s for s in stored}
    today_local = datetime.now().astimezone().strftime("%Y-%m-%d")
    window_includes_today = (to_b is None) or (to is None) or (to >= today_local)
    if window_includes_today and include_local_live:
        for s in await get_sessions_cached():
            if _session_in_filters(s, from_b, to_b, agents, models, projects):
                merged[(local_id, s.get("agent"), s.get("id"))] = s  # live wins over stored (local)
    sessions = list(merged.values())
    by_agent = {}; by_day = {}; by_day_project: Dict[str, Dict[str, Any]] = {}; by_model = {}
    for s in sessions:
        agent = s["agent"]
        ...
        # Optionally fold each session's delegated (subagent) usage into its own
        # headline numbers. Off by default → the aggregation below stays
        # byte-for-byte identical. When on, the subagent input/output/cached are
        # added to the parent's tokens and the per-session cost contribution gains
        # the delegated cost (the precomputed delegated_cost when present, else a
        # fresh calculate_cost on the delegated tokens for the session's model).
        if include_delegated and st:
            di = st.get("delegated_input", 0) or 0
            do = st.get("delegated_output", 0) or 0
            dc = st.get("delegated_cached", 0) or 0
            if di or do or dc:
                st = dict(st)
                st["input"] = st.get("input", 0) + di
                st["output"] = st.get("output", 0) + do
                st["cached"] = st.get("cached", 0) + dc
                st["total"] = st.get("total", 0) + di + do + dc
                deleg_cost = s.get("delegated_cost")
                if deleg_cost is None:
                    deleg_cost = calculate_cost(
                        s.get("model"), di, do, dc,
                        provider=s.get("provider"), endpoint=s.get("endpoint"),
                        billing_mode=s.get("billing_mode"),
                    )
                scost = scost + (deleg_cost or 0)
        ...
        proj_name = (os.path.basename((proj or "").replace("\\", "/").rstrip("/")) or proj or "unknown").strip()
        if day not in by_day_project:
            by_day_project[day] = {}
        if proj not in by_day_project[day]:
            by_day_project[day][proj] = {"project": proj, "name": proj_name, "total": 0, "cost": 0.0, "session_count": 0}
        by_day_project[day][proj]["total"] += st.get("total", 0)
        by_day_project[day][proj]["cost"] += scost
        by_day_project[day][proj]["session_count"] += 1
    ...
    return {
        ...
        "by_day": sorted_days,
        "by_day_project": {
            day: sorted(projs.values(), key=lambda p: p["total"], reverse=True)
            for day, projs in by_day_project.items()
        },
        "by_model": by_model,
        "by_skill": by_skill,
        ...
        "coverage": history_store.coverage(device_ids=device_ids),
