...
**TokenAnalytics** — free, open-source, 100% local.
...
[![Website](https://img.shields.io/badge/Website-tokenanalytics.app-blue)](https://tokenanalytics.app)
[![GitHub Stars](https://img.shields.io/github/stars/2netgr/tokenanalytics?style=social)](https://github.com/2netgr/tokenanalytics)

**TokenAnalytics** is a free, open-source, 100% local observability dashboard that tracks **token usage**, **LLM costs**, **tool calls**, **session traces**, and **reasoning steps** across all your AI coding agents — in one unified place. No signup. No cloud. Your logs never leave your machine.

🌐 **Website & Docs:** [https://tokenanalytics.app](https://tokenanalytics.app)  
🖥️ **macOS/Linux:** `curl -fsSL https://raw.githubusercontent.com/2netgr/tokenanalytics/main/install.sh | bash`
🧰 **Windows:** `irm https://raw.githubusercontent.com/2netgr/tokenanalytics/main/install.ps1 | iex`
🐙 **GitHub:** [github.com/2netgr/tokenanalytics](https://github.com/2netgr/tokenanalytics)
...
TokenAnalytics answers all of that — locally, instantly, for free.

| Problem                                                | TokenAnalytics Solution                     |
| ------------------------------------------------------ | ------------------------------------------- |
...
TokenAnalytics reads session logs from these agents automatically.
...
More agents added regularly. [Request support for your agent →](https://github.com/2netgr/tokenanalytics/issues)
...
Run TokenAnalytics on the same host as Hermes — we read `$HERMES_HOME` (or `~/.hermes/` if unset) locally, no remote-DB mode yet.

### Hermes Dashboard plugin (`:9119` → `:3000`)

If you run Hermes's own web dashboard (`hermes dashboard`, port `9119`), install the plugin so TokenAnalytics shows up as a tab inside it — one port to remember, deep-link cards to every TT page.

**Standalone install** (recommended — uses Hermes's own plugin manager):

```bash
hermes plugins install 2netgr/tokenanalytics-hermes-plugin
hermes dashboard
```
...
```powershell
irm https://tokenanalytics.app/install.ps1 | iex
```

### Option 2: Clone & run

```bash
git clone https://github.com/2netgr/tokenanalytics.git
cd tokentelemetry
...
Set a spend or token limit on any project — monthly, weekly, or rolling 30-day, overall and per agent. Budgets are **observational**: TokenAnalytics tracks this period's spend against your limit but never blocks an agent.
...
TokenAnalytics stores lightweight state in `~/.tokenanalytics/`:
...
> **Upgrading from TokenAnalytics?** The legacy `~/.tokentelemetry/` directory
> is auto-adopted — the backend reads it as a fallback when `~/.tokenanalytics/`
> doesn't exist, so existing users keep their data with no migration step.

### Choosing where data is stored

Prefer to keep your system drive clear, or isolate dev-tool state on a secondary
drive? Point TokenAnalytics's data directory anywhere:

- **Launcher flag** — `start.sh --data-dir /mnt/d/tt-data` (or `-d`). The folder
  is created on first write.
- **Environment** — set `TOKENANALYTICS_DATA_DIR=/mnt/d/tt-data` (or
  `TOKENANALYTICS_HOME`) before launching. Used verbatim — that exact folder
  becomes the store (no `.tokenanalytics` suffix appended). An explicit
  `--data-dir` flag wins over it. The legacy `TOKENTELEMETRY_DATA_DIR` /
  `TOKENTELEMETRY_HOME` names still work as aliases for backward compatibility.
...
relocates *all* state. The default remains `~/.tokenanalytics/`.

### Privacy & outbound calls

Your AI-usage data — logs, sessions, tokens, costs, prompts, file paths, and
project names — **never leaves your machine**. TokenAnalytics makes two
...
2. **Anonymous product-usage telemetry** (on by default, one-click off) —
   ...
   [tokenanalytics.app/privacy](https://tokenanalytics.app/privacy) and
...
TokenAnalytics is **local-first** and binds to `127.0.0.1` by default so that your agent logs, prompts, and costs never leave the machine. Remote access is an opt-in feature with clear security boundaries (loopback is always exempt; non-loopback requests require a token).
...
**On the remote machine** (where the agent logs live — this is required because TokenAnalytics reads files locally):
...
**Q: Does TokenAnalytics send any data to the cloud?**  
...
A: Claude Code writes JSONL session logs to `~/.claude/`. TokenAnalytics watches those files and parses token counts, tool calls, and reasoning in real time.
...
**Q: Is there a cost to use TokenAnalytics?**  
A: No. It is free and open-source under the MIT license.

**Q: How is TokenAnalytics different from Langfuse, LangSmith, or Helicone?**  
A: Those tools require you to instrument your code, create an account, and send data to their cloud. TokenAnalytics is 100% local, zero-config, and works by reading the log files your agents already write — no SDK, no API key, no cloud.

**Q: Can I monitor Gemini CLI token usage?**  
A: Yes. TokenAnalytics supports Gemini CLI and shows token counts, costs, and session traces for Google's Gemini models (Gemini 2.0 Flash, Gemini 1.5 Pro, etc.).
...
A: Yes — run TokenAnalytics on the same host (it reads local files). See the **[Remote Access](#remote-access)** section above for the two supported methods:
...
A: No. Hermes Agent is the [open-source agent framework](https://github.com/NousResearch/hermes-agent); Hermes-3 is a family of fine-tuned models. TokenAnalytics observes the agent — it can be running any model.

---

## Comparisons

| Feature             | TokenAnalytics | Langfuse | LangSmith | Helicone |
| ------------------- | -------------- | -------- | --------- | -------- |
| 100% Local          | ✅             | ❌       | ❌        | ❌       |
| Zero config         | ✅             | ❌       | ❌        | ❌       |
| No signup           | ✅             | ❌       | ❌        | ❌       |
...
| **TokenAnalytics**                  | ✅            | ✅     | `/hermes` dashboard |

Know of another? [Open an issue](https://github.com/2netgr/tokenanalytics/issues) and we'll update this.
...
**No sessions showing:** Run an agent (Claude Code, Gemini CLI, etc.) first — TokenAnalytics needs existing log files.  
...
Want to add support for a new agent? [Open an issue](https://github.com/2netgr/tokenanalytics/issues) with the agent name and log format.
...
**Hemanth Vasi**  
🌐 [tokenanalytics.app](https://tokenanalytics.app)  
...
Have an idea, found a bug, or just want to share how you're using TokenAnalytics? Two ways in:

- 💬 **[GitHub Discussions](https://github.com/2netgr/tokenanalytics/discussions)** — ideas, Q&A, show-and-tell
- 🐛 **[Issues](https://github.com/2netgr/tokenanalytics/issues)** — bugs and concrete feature requests

There's also a feedback button inside the app (bottom-right of every page).

---

_If you find TokenAnalytics useful, please ⭐ star this repo — it helps others discover it!_
