...
custom domain `tokenanalytics.app`). It sells the product but doesn't *teach* it.
...
- **Post-install / in-app** — the local app gets a global **Docs** link plus
  ...
  page (e.g. the Hermes page's help icon opens `tokenanalytics.app/docs/hermes`
  in the browser).
...
└── Docs link in nav + per-page "?" help icons → deep-link to tokenanalytics.app/docs/<feature>
```
...
- **Subdirectory, not subdomain** — `tokenanalytics.app/docs` + `/resources`, not
  `docs.tokenanalytics.app`. A subdirectory inherits the main domain's accumulated
