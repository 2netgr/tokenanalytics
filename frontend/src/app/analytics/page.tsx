...
import {
  ...
  Zap, DollarSign, Cpu, GitBranch, X, Folder,
} from "lucide-react";
import {
...
import { isAgentVisible, AGENTS_HIDDEN_BY_DEFAULT } from "@/lib/agent-visibility";
import { useTheme } from "@/components/ThemeProvider";
import DeviceSelector, { type DeviceSelection } from "@/components/DeviceSelector";
...
interface DayProject { project: string; name: string; total: number; cost: number; session_count: number; }
...
interface AnalyticsData {
  ...
  by_day_project?: Record<string, DayProject[]>;
...
export default function AnalyticsPage() {
  ...
  const [device, setDevice] = useState<DeviceSelection>("local");
  const [includeDelegated, setIncludeDelegated] = useState(true);
  ...
  const queryPath = useMemo(() => {
    ...
    // Omit device=local so the default call stays byte-for-byte identical to
    // the single-device behaviour; only send it when scoped elsewhere.
    if (device !== "local") p.set("device", device);
    // Off by default → omit the param so the default call is unchanged.
    if (includeDelegated) p.set("include_delegated", "true");
    const qs = p.toString();
    return qs ? `/analytics?${qs}` : "/analytics";
  }, [range, granularity, customFrom, customTo, selAgents, selModels, device, includeDelegated]);
  ...
  const [hiddenAgents, setHiddenAgents] = useState<Set<string>>(new Set(AGENTS_HIDDEN_BY_DEFAULT));
  useEffect(() => {
    const sync = () => {
      const hidden = new Set<string>();
      for (const k of AGENTS_HIDDEN_BY_DEFAULT) {
        if (!isAgentVisible(k)) hidden.add(k);
      }
  ...
  const agentOptions = (useResource<string[]>("/agents").data ?? []).filter((k) => !hiddenAgents.has(k));
  ...
  return (
    <div className="px-8 py-8 max-w-[1600px] mx-auto space-y-10 pb-20">
      {clickedDay && (
        <DayProjectModal
          day={clickedDay}
          projects={data.by_day_project?.[clickedDay] ?? []}
      ...
      <PageHeader
        ...
        icon={<img src="/icon-ta.png" alt="" className="h-5 w-5 rounded-[5px]" />}
        actions={
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-3 -mt-4">
        <DeviceSelector
          value={device}
          onChange={(d) => { setDevice(d); trackEvent("analytics.filtered", { dimension: "device" }); }}
        />
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] uppercase tracking-[0.16em] text-[var(--tt-fg-dim)]">Subagents</span>
          <button
            type="button"
            role="switch"
            aria-checked={includeDelegated}
            onClick={() => {
              setIncludeDelegated(v => !v);
              trackEvent("analytics.filtered", { dimension: "include_delegated" });
            }}
            className={cn(
              "px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] rounded-[var(--tt-radius)] border transition-colors",
              includeDelegated
                ? "border-[var(--tt-brand)] text-[var(--tt-fg)] bg-[var(--tt-brand)]/10"
                : "border-[var(--tt-border)] text-[var(--tt-fg-dim)] hover:text-[var(--tt-fg)]"
            )}
          >
            {includeDelegated ? "Included" : "Include"}
          </button>
        </div>
      ...
      {data.coverage?.earliest && (
        <div className="flex items-start gap-2 text-[11px] text-[var(--tt-fg-dim)] rounded-[var(--tt-radius)] border border-[var(--tt-border)] bg-[var(--tt-sunken)] px-3 py-2 -mt-4">
          ...
          <span>
            ...
            TokenAnalytics only records sessions present on disk from its first run — usage before then can't be
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2" padding="lg">
          ...
          <p className="text-[10px] text-[var(--tt-fg-dim)] mt-2 text-right tracking-wide">
            Click any point to see project breakdown
          </p>
        </Card>
...
function DayProjectModal({
  ...
  return (
    ...
    >
      ...
      {/* panel */}
      <div
        className="relative z-10 w-full max-w-md rounded-[var(--tt-radius-lg)] border border-[var(--tt-border)] bg-[var(--tt-panel)] shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--tt-border)]">
          <div>
            <div className="text-[13px] font-semibold text-[var(--tt-fg)]">{day}</div>
            <div className="text-[11px] text-[var(--tt-fg-dim)] mt-0.5">
              {total.toLocaleString()} tokens across {projects.length} project{projects.length !== 1 ? "s" : ""}
            </div>
          </div>
          <button
            onClick={onClose}
            className="h-7 w-7 grid place-items-center rounded-[var(--tt-radius)] text-[var(--tt-fg-dim)] hover:text-[var(--tt-fg)] hover:bg-[var(--tt-sunken)] transition-colors"
          >
            <X size={14} />
          </button>
        </div>
        <ul className="max-h-[60vh] overflow-y-auto divide-y divide-[var(--tt-border)]">
          {projects.length === 0 ? (
            <li className="px-5 py-6 text-center text-[12px] text-[var(--tt-fg-dim)]">No project data for this day.</li>
          ) : projects.map((p) => {
            const pct = total > 0 ? (p.total / total) * 100 : 0;
            return (
              <li key={p.project} className="px-5 py-3">
                <div className="flex items-start justify-between gap-3 mb-1.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <Folder size={13} className="text-[var(--tt-brand)] shrink-0" />
                    <span className="text-[12px] font-semibold text-[var(--tt-fg)] truncate" title={p.name}>{p.name}</span>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-[12px] tabular font-semibold text-[var(--tt-fg)]">{p.total.toLocaleString()}</div>
                    <div className="text-[10px] tabular text-amber-300">${p.cost.toFixed(3)}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1 rounded-full bg-[var(--tt-sunken)] overflow-hidden">
                    <div
                      className="h-full rounded-full bg-[var(--tt-brand)]"
                      style={{ width: `${pct.toFixed(1)}%` }}
                    />
                  </div>
                  <span className="text-[10px] tabular text-[var(--tt-fg-dim)] w-10 text-right">{pct.toFixed(0)}%</span>
                  <span className="text-[10px] text-[var(--tt-fg-dim)]">·</span>
                  <span className="text-[10px] text-[var(--tt-fg-dim)]">{p.session_count} session{p.session_count !== 1 ? "s" : ""}</span>
                </div>
              </li>
            );
          })}
