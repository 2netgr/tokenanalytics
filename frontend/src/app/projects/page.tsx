...
import {
  ...
  LayoutGrid, List as ListIcon, ArrowUpDown, FolderOpen, ChevronDown,
} from "lucide-react";
...
type SortKey = "last_activity" | "sessions" | "tokens" | "cost" | "name";
...
export default function ProjectsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    // Split into projects with cost vs $0.00
    return {
      visible: sorted.filter((p) => (p.tokens?.cost ?? 0) > 0),
      zero: sorted.filter((p) => (p.tokens?.cost ?? 0) === 0),
    };
  ...
  return (
    <div className="px-8 py-8 max-w-[1600px] mx-auto space-y-8 pb-20">
      ...
      {loading ? (
      ...
      ) : filtered.visible.length === 0 && filtered.zero.length === 0 ? (
        <Card>
          <EmptyState
            icon={<Search size={20} />}
        ...
        <Section>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.visible.map((p) => <ProjectCard key={p.path} project={p} />)}
            {showZeroCost && filtered.zero.map((p) => <ProjectCard key={p.path} project={p} />)}
          </div>
          {!showZeroCost && filtered.zero.length > 0 && (
            <div className="mt-4 flex justify-center">
              <button
                onClick={() => setShowZeroCost(true)}
                className="flex items-center gap-2 px-4 py-2 rounded-[var(--tt-radius)] border border-[var(--tt-border)] text-[12px] text-[var(--tt-fg-muted)] hover:text-[var(--tt-fg)] hover:border-[var(--tt-border-strong)] transition-colors"
              >
                <ChevronDown size={14} />
                Show {filtered.zero.length} more ($0.00)
              </button>
            </div>
          )}
        </Section>
        ...
        <Card padding="none">
          <Table>
            ...
            <TBody>
              {[...filtered.visible, ...(showZeroCost ? filtered.zero : [])].map((p) => (
