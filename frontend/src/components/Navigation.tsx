...
import {
  LayoutDashboard, Folder, BarChart3, Settings2,
  PanelLeftOpen, PanelLeftClose, Zap,
} from "lucide-react";
import { useResource } from "@/lib/api";
import { ALL_AGENT_KEYS, getAgent } from "@/lib/agents";
import { isAgentVisible, isLocalModelsPageVisible, AGENTS_HIDDEN_BY_DEFAULT } from "@/lib/agent-visibility";
...
export default function Navigation({ isCollapsed, setIsCollapsed }: NavigationProps) {
  ...
  const [hiddenAgents, setHiddenAgents] = useState<Set<string>>(
    new Set(AGENTS_HIDDEN_BY_DEFAULT)
  );
  ...
  useEffect(() => {
    const sync = () => {
      ...
      setShowLocalModels(isLocalModelsPageVisible());
    };
    sync();
  ...
  const LINKS = [
    ...BASE_LINKS,
    ...(showLocalModels ? [{ name: "Local Models", href: "/local-models", icon: Zap }] : []),
  ];
  ...
  return (
    ...
    >
      {/* Brand */}
      <div className={cn("flex items-center gap-2.5 px-2 py-3 mb-3", isCollapsed && "justify-center px-0")}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/icon-ta.png" alt="TokenAnalytics" width={36} height={36} className="rounded-[10px] shrink-0" />
        {!isCollapsed && (
          <div className="min-w-0 leading-none">
            <div className="text-[15px] font-bold tracking-[-0.02em]">
              <span className="text-[var(--tt-fg)]">Token</span>
              <span className="bg-gradient-to-r from-blue-400 to-violet-500 bg-clip-text text-transparent">Analytics</span>
            </div>
            <div className="text-[10px] text-[var(--tt-fg-dim)] tracking-[0.12em] mt-0.5">.app</div>
          </div>
        )}
      </div>
      ...
      <div className="mt-auto space-y-3">
        {!isCollapsed && visibleAgents.length > 0 && (
          <div className="rounded-[var(--tt-radius-lg)] border border-[var(--tt-border)] bg-[var(--tt-sunken)] p-3">
            <div className="text-[9px] font-semibold uppercase tracking-[0.2em] text-[var(--tt-fg-dim)] mb-2.5 flex items-center justify-between">
              <span>Connected</span>
              <span className="tabular text-[var(--tt-fg-muted)]">{visibleAgents.length}</span>
            </div>
